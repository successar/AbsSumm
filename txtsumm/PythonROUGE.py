...
def PythonROUGE(guess_summ_list,ref_summ_list,ngram_order=2):
    ...
    ROUGE_path = PROJECT_DIR + 'txtsumm/ROUGE-RELEASE-1.5.5/ROUGE-1.5.5.pl'
    data_path = PROJECT_DIR + 'txtsumm/ROUGE-RELEASE-1.5.5/data'
...
if __name__ == '__main__':
    out_dir = PROJECT_DIR + 'txtsumm/output'
    mod_dir = PROJECT_DIR + 'txtsumm/models'
    ...
    for i,f in enumerate(files) :
        summs.append(out_dir + '/' + f)
        refs = []
        for i in range(4) :
            refs.append(mod_dir + '/' + mod_files[j])
            j = j+1
        actuals.append(refs)
    ...
    recall_list,precision_list,F_measure_list = PythonROUGE(summs,actuals,1)
