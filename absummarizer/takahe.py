...
from networkx.drawing.nx_agraph import write_dot as nx_write_dot
...
class word_graph:
    ...
    def pre_process_sentences(self):
        ...
        for i in range(self.length):
            ...
            self.sentence[i] = re.sub(r' +', ' ', self.sentence[i])
    ...
    def write_dot(self, dotfile):
        ...
        nx_write_dot(self.graph, dotfile)
    #-B-----------------------------------------------------------------------B-
...
def getSentences(sentences, cluster_name = "cluster", mode="Abstractive") :
    #print '\n'
    #print sentences
    gensent = []
    if mode == "Abstractive" :
        compresser = word_graph(sentences, lang = 'en',punct_tag = "PUNCT" , nb_words=8)
        #compresser.write_dot(cluster_name + '.dot')
        
        candidates = compresser.get_compression(100)
        for cummulative_score, path in candidates:
            gensent.append(' '.join([u[0] for u in path]))

    else:
        gensent = sentences

    if len(gensent) > 0:
        return textrank(gensent)
    else :
        return []
...
def textrank(sentences):
    ...
    scores = bow_matrix * np.array(scores)

    return sorted(((s,scores[i]) for i,s in enumerate(sentences)),
                  reverse=True)


def solveILPFactBased(clusters, languageModel, mode="Abstractive" ):
    # Create a new model
    ...
    for i,sentences in enumerate(clusters):
        docs.append([])
        informativeness.append([])
        lengths.append([])
        for sent in sentences:
            docs[i].append(sent[0])
            informativeness[i].append(sent[1])
            lengths[i].append(len(sent[0].split(' ')))
    ...
    for i in xrange(len(docs)):
        ...
        for j in xrange(len(docs[i])):
            var=LpVariable("var_"+str(i)+"_"+str(j),cat=LpBinary)
            varlist[i].append(var)
    
    if mode=="Abstractive":
        obj = []
        for i in xrange(len(docs)):
            for j in xrange(len(docs[i])):
                obj.append(informativeness[i][j]*linguistic[i][j]*varlist[i][j]*1.0/lengths[i][j])
        m += lpSum(obj), "Obj function"  
    else:
        obj = []
        for i in xrange(len(docs)):
            for j in xrange(len(docs[i])):
                obj.append(informativeness[i][j]*varlist[i][j]*1.0/lengths[i][j])
        m += lpSum(obj), "Obj function" 
    ...
    for i in xrange(len(docs)):
        cons1 = []
        for j in xrange(len(docs[i])):
            cons1.append(varlist[i][j])
        m += lpSum(cons1) <= 1.0, "Cluster"+str(i)

    cons2 = []
    for i in xrange(len(docs)):
        for j in xrange(len(docs[i])) :
            cons2.append(lengths[i][j] * varlist[i][j])

    m += lpSum(cons2) <= 200, "Total Length"
    ...
    for v in m.variables():
        if v.varValue == 1.0:
            ind = v.name.split("_")
            solutionList.append((int(ind[1]), int(ind[2])))

    solutionList = sorted(solutionList, key=lambda x: x[0])
    solutionList = [docs[x][y] for (x,y) in solutionList]
    return solutionList
